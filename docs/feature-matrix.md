...
5. **MCP integration** — Exposes 27 MCP tools, 7 redacted resources, and 2 prompts for read-only inventory, schema inspection, trace review, draft contract inference, duplicate detection, section resolution, diagnostics, media inspection, Creator OS planning, and safe frontier surface review. Write flows (publish, schedule, draft, config, auth login, admin writes, imports, and native media/live mutations) are intentionally excluded from the MCP surface for safety.
