...
describe("getSubscriberCount", () => {
  it("returns subscriber count from checklist", async () => {
    ...
    const count = await getSubscriberCount("https://test.substack.com", material, fetchFn);

    assert.equal(count, 42);
  });

  it("returns 0 when subscriber_count field is missing", async () => {
    const fetchFn = fakeFetch(200, JSON.stringify({})) as unknown as typeof fetch;
  ...
  it("throws on network error", async () => {
    const fetchFn = (() => Promise.reject(new Error("Network failure"))) as unknown as typeof fetch;
