...
import { classifyMutationError, outcomeUnknown, requireReconciliationBeforeReplay } from "./mutation-outcome.js";
...
describe("mutation outcomes", () => {
  it("represents transport failures as outcome-unknown", () => {
    ...
    expect(result).toEqual({ kind: "outcome-unknown", operation: "draft.create", reason: "socket closed" });
  ...
  it("requires reconciliation before replaying unknown outcomes", () => {
    expect(() => requireReconciliationBeforeReplay(outcomeUnknown("post.publish", "timeout"))).toThrow();
