...
async function createDraftInBrowser(
  ...
  await recordStep(trace, "verify-publish-review-screen", async () => {
    ...
    if (!isReviewUrl) {
      log.warn(
        { url },
        'Current URL does not look like a publish review screen. Expected URL containing "/publish/" or "/post/".',
      );
    }
