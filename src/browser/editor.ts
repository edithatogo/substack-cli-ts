...
export async function pasteHtmlIntoEditor(
  ...
  return page.evaluate(
    ({ html, markdown, document }) => {
      ...
      if (!target) {
        return { ok: false, target: null, reason: "editor contenteditable not found" };
      }
      ...
      return inserted
        ...
        : { ok: false, target: describeElement(target), reason: "browser refused html insertion" };
    },
    payload,
  );
}
...
function findMainEditor(): HTMLElement | null {
  ...
  return candidates
    .map((element) => ({
      element,
      score: (element.textContent?.length ?? 0) + element.getBoundingClientRect().height,
    }))
