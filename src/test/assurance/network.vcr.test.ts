...
import { FixtureTransport, parseMockTransportFixture } from "../../substack-api/fixture-transport.js";

describe("network interaction VCR replay", () => {
  it("replays recorded responses and fails closed for unrecorded routes", async () => {
    const transport = new FixtureTransport(
      parseMockTransportFixture(JSON.stringify({
        scenario: "vcr-read-replay",
        generatedAt: "2026-08-08T00:00:00.000Z",
        calls: [{ method: "GET", url: "https://example.test/api/me", responses: [{ status: 200, body: { id: 7 } }] }],
      })),
    );
