...
| D006 | Confirm the exact draft create/update/fetch request bodies and returned IDs for the live Substack editor. | Track 08 implementation       | Track 08      | Track 08, 11, 12    | Open; local capture, comparison, fixture, contract inference, matrix, matrix-fixture, matrix-compare, duplicate lookup, section-resolution, and inspect tools now available |
...
- Add a local draft endpoint capture fixture once D006 is resolved; `api draft contract`, `api draft contract-matrix`, `api draft fixture`, and `api draft compare` now support local diffing and inference.
...
- Match prepared drafts against the read-only inventory with `api draft duplicates` to catch obvious collisions before any live write path.
- Resolve section metadata with `api draft section` so draft payloads can carry the correct sectionId.
- Use `api draft inspect` when you need a single report covering compatibility, section resolution, duplicates, and the local draft plan.
